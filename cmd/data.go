...
func mkDataFile(input string, outdir string) {
	...
	for i, row := range sheet.Rows {
		// 項番
		// columnA := strings.TrimSpace(row[0])
		...
		if cellslen < 4 {
			er(fmt.Sprintf("%d, %d, \"%s\", \"%s\"", i, cellslen, row.Cells[0].String(), row.Cells[1].String()))
		}
